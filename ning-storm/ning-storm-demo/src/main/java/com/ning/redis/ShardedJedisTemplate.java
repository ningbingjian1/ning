...
public class ShardedJedisTemplate {
    // 2018-01-25 因为历史原因 ，配置改成集群模式  但是还只能使用原来的分片文件的配置名
    ...
    public static ShardedJedisTemplate getInstance(String configFilePath) {
        if (jedisTemplates.get(configFilePath) == null) {
            synchronized (ShardedJedis.class) {
                if (jedisTemplates.get(configFilePath) == null) {
                    JedisCluster pool = ClusterJedisPoolFactory.createJedisPool(configFilePath);
                    ShardedJedisTemplate template  = new ShardedJedisTemplate(pool);
                    jedisTemplates.put(configFilePath, template);
                }
            }
        }
    ...
    protected ShardedJedisTemplate(JedisCluster pool) {
        this.jedisPool = pool;
    }
    ...
    public void execute(JedisActionNoResult jedisAction) {
        JedisCluster jedis = this.jedisPool;
        boolean broken = false;
        try {
     ...
     */
    protected void closeResource(JedisCluster jedis, boolean connectionBroken) {
        if (jedis != null) {
            try {
                if (connectionBroken) {
    ...
    public interface JedisActionNoResult {
        void action(JedisCluster jedis);
    }
    ...
    public Boolean del(final String key) {
        return execute(new JedisAction<Boolean>() {
            ...
            @Override
            public Boolean action(JedisCluster jedis) {
    ...
    public String get(final String key) {
        return execute(new JedisAction<String>() {
            ...
            @Override
            public String action(JedisCluster jedis) {
    ...
    public byte[] get(final byte[] key) {
        return execute(new JedisAction<byte[]>() {
            ...
            @Override
            public byte[] action(JedisCluster jedis) {
                return jedis.get(key);
            }
        });
    }
    ...
    public void set(final String key, final String value) {
        execute(new JedisActionNoResult() {
            ...
            @Override
            public void action(JedisCluster jedis) {
                jedis.set(key, value);
            }
        });
    }
    ...
    public Long incr(final String key) {
        return execute(new JedisAction<Long>() {
            @Override
            public Long action(JedisCluster jedis) {
    ...
    public Double zscore(final String key, final String member) {
        return execute(new JedisAction<Double>() {
            ...
            @Override
            public Double action(JedisCluster jedis) {
    ...
    public Map<String, String> hgetall(final String key) {
        return execute(new JedisAction<Map<String, String>>() {
            @Override
            public Map<String, String> action(JedisCluster jedis) {
    ...
    public List<byte[]> lrange(final byte[] key, final int i, final int j) {
        return execute(new JedisAction<List<byte[]>>() {
            @Override
            public List<byte[]> action(JedisCluster jedis) {
