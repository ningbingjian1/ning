...
public class JedisScriptExecutor {
    ...
    public void loadFromFile(final String scriptPath) throws JedisDataException {
        ...
        try {
            InputStream script = BaseJedisPoolFactory.class.getClassLoader().getParent()
                    .getResourceAsStream(scriptPath);
            if (script == null) {
                script = BaseJedisPoolFactory.class.getResourceAsStream(scriptPath);
            }
            if (script == null) {
                throw new RuntimeException("Cannot find " + scriptPath + " !!!");
            }
            scriptContent = IOUtils.toString(script);
        } catch (IOException e) {
